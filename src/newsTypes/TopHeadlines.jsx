...
export default function TopHeadlines() {
  ...
  // ****** Functions ******
  const getData = async()=> {
    const response = await fetch("https://newsapi.org/v2/top-headlines?country=us&apiKey=7e1628d613e741f0a7d9fdd4d88e2558")
    if(response.ok) {
  ...
  if(loading){
    ...
    <div>
      <span>Some Error Occured Please Check your Internet Connection or Try again Later </span>
    </div>
  }
...
<>
  ...
  <div className='mx-0 lg:mx-24 p-9'>
      {news.map((data,index)=>(
        <div className='mt-20 bg-white p-3 md:h-64 rounded-xl grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 md:mr-44 lg:mr-72  h-fit  hover:shadow-2xl hover:scale-105 transition mb-6' key={index}>
          <div id="image" className='p-3'>
            <img src={data.urlToImage} alt={index} className='border-2 border-white w-fit lg:w-52 h-52 shadow-2xl'/>
          </div>
